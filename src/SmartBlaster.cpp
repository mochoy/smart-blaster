...
void SmartBlaster::PWM (uint8_t toPWM) {    //0 = flywheels, 1 = pusher
  if (_isFlywheelPWM || _isPusherPWM) {
    ...
    const uint8_t PWM_IN_PIN = isFlywheel ? _FLYWHEEL_PWM_POT_PIN : (_isPusherPWM ? _PUSHER_PWM_POT_PIN : zero);
    const uint8_t PWM_OUT_PIN = isFlywheel ? _FLYWHEEL_OUT_PIN : (_isPusherPWM ? _PUSHER_OUT_PIN : zero);
    const uint32_t MOTOR_ACCEL_TIME = isFlywheel ? _FLYWHEEL_MOTOR_ACCEL_TIME : (_isPusherPWM ? _PUSHER_MOTOR_ACCEL_TIME : (uint32_t)zero); 
